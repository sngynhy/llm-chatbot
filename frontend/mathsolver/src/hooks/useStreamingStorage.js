...
export function useStreamingStorage() {
  ...
  // 특정 chatId 메시지 초기화
  const resetMessages = useCallback((chatId) => {
    dispatch({
  ...
  const resetAllMessages = useCallback(() => {
    dispatch({ type: actionType.resetAllMessages });
  }, []);
